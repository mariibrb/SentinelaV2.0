...
def gerar_resumo_uf(df_saida, writer, df_entrada=None):
    # Conforme regra: Manter o restante igual, alterando apenas a lógica de soma e cor.
    ...
    def preparar_tabela(df_origem, tipo):
        ...
        # Validação de CFOP (Filtra operações comerciais 1xxx, 2xxx, 3xxx / 5xxx, 6xxx, 7xxx)
        ...
        # Mapeamento da IE Substituta (Busca no DF Validado)
    ...
    for df_t, start_c, title in [(res_s, 0, "1. SAÍDAS"), (res_e, 7, "2. ENTRADAS"), (res_saldo, 14, "3. SALDO")]:
        ...
        for r_idx, row in enumerate(df_t.values):
            uf = str(row[0]).strip()
            # Volta da regra de cor: Laranja se existir IE na Saída para aquela UF
