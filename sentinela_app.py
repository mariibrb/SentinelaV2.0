...
def carregar_base_clientes():
    # Lê a planilha de empresas na raiz do projeto
    try:
        df = pd.read_csv('Clientes Ativos.xlsx - EMPRESAS.csv')
        return df
    except:
        st.error("Erro: Planilha 'Clientes Ativos.xlsx - EMPRESAS.csv' não encontrada na raiz do projeto.")
        return pd.DataFrame()
...
if selecao:
    cod_id = int(selecao.split(" - ")[0])
    dados = df_clientes[df_clientes['CÓD'] == cod_id].iloc[0]
    cnpj_auditado = dados['CNPJ']

    st.info(f"🧡 Auditando: {dados['RAZÃO SOCIAL']} | CNPJ: {cnpj_auditado}")
    ...
    if regime:
        ...
        with col1:
            ...
            xmls = st.file_uploader("Upload de XMLs ou ZIPs (Triagem Automática)", type=['zip', 'xml'], accept_multiple_files=True)
        ...
        if st.button("🚀 GERAR RELATÓRIO"):
            with st.spinner("🧡 Sentinela está separando e processando tudo..."):
                # A função abaixo agora separa tudo pelo CNPJ
                df_xe, df_xs = extrair_dados_xml_recursivo(xmls, cnpj_auditado)
                
                # Gera o Excel final
                relat = gerar_excel_final(df_xe, df_xs, None, as_f, ge, gs, cod_id, regime, is_ret)
                
                st.success("Concluído! 🧡")
                st.download_button("💾 BAIXAR AGORA", relat, f"Sentinela_{cod_id}.xlsx", use_container_width=True)
