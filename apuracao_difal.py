...
def gerar_resumo_uf(df, writer):
    ...
    # 1. Filtro rigoroso: Notas autorizadas
    df_validas = df_temp[
    ...
    def preparar_tabela(df_origem):
        ...
        # BUSCA IE (Coluna B): Mapeia o número extraído recursivamente no Core
    ...
    for df_t, start_c, title in [(res_s, 0, "1. SAÍDAS"), (res_e, 7, "2. ENTRADAS"), (res_saldo, 14, "3. SALDO")]:
        ...
        for r_idx, row in enumerate(df_t.values):
            ...
            for c_idx, val in enumerate(row):
                fmt = orange_num if tem_ie and isinstance(val, (int, float)) else orange_fill if tem_ie else num_fmt if isinstance(val, (int, float)) else border_fmt
