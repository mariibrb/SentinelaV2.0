...
def gerar_resumo_uf(df, writer):
    ...
    # 1. Filtro rigoroso: Somente notas autorizadas
    df_validas = df_temp[
    ...
    def preparar_tabela(df_origem):
        ...
        # BUSCA IE (Coluna B): Mapeia o número 813032863112 extraído no Core
    ...
    # 4. Cálculo do Saldo Líquido (Regra: Só abate se tiver IE na Saída)
    ...
    for c_xml, c_fin in colunas_calculo:
    ...
    # Escrever os blocos lado a lado (A-F, H-M, O-T)
    for df_t, start_c, title in [(res_s, 0, "1. SAÍDAS"), (res_e, 7, "2. ENTRADAS"), (res_saldo, 14, "3. SALDO")]:
        ...
        for r_idx, row in enumerate(df_t.values):
            uf = str(row[0]).strip()
            # Identifica se pinta de laranja baseado na IE da Saída
            ...
            for c_idx, val in enumerate(row):
                ...
                if c_idx == 1: # Escreve IE como String para o número 813032863112 aparecer correto
